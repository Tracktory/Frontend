...
export function EmploymentPreferenceSophomorePage({ navigation }: Props) {
  ...
  const toggleCompanyType = useOnboardingStore((state) => state.togglePreferredCompanyTypeSophomore);
  const selectedValues = useOnboardingStore((state) => state.employmentValuesSophomore);
  const toggleValue = useOnboardingStore((state) => state.toggleEmploymentValueSophomore);
  ...
  const handleToggleValue = (value: string) => {
    ...
    if (!alreadySelected && valuesMaxReached) {
      console.log('[Onboarding] 2학년+ 중요 가치는 최대 3개까지 선택할 수 있습니다.');
      return;
    }
    toggleValue(value);
  };

  const canProceed = selectedCompanyTypes.length > 0 && selectedValues.length > 0;
  ...
  return (
    <View style={styles.screen}>
      <ScrollView style={styles.scrollArea} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Text style={styles.screenId}>ON-SCR-06b | ON-008, ON-009</Text>
        <ProgressBar progress={0.7} />
        <Text style={styles.title}>취업 선호도를 알려주세요</Text>
        <Text style={styles.subtitle}>회사 유형과 중요 가치를 선택해주세요</Text>
        ...
        <View style={styles.chipGroup}>
          {COMPANY_TYPE_OPTIONS.map((type) => (
            <InterestChip
              key={type}
              label={type}
              selected={selectedCompanyTypes.includes(type)}
      ...
      <View style={styles.bottomArea}>
        <Button
          ...
          onPress={() => canProceed && navigation.navigate('ExperiencedFieldSophomore')}
        />
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 14, fontWeight: '600', color: '#404040', marginBottom: 12 },
  chipGroup: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 24 },
